...
#ifdef ASTDEBUG
inline void print_ast(const ast_node_t& node, size_t depth = 0);
...
struct maybe_node_t {
    bool is_none;
    ast_node_t node;
    inline maybe_node_t() : is_none(true) {}

    inline maybe_node_t(const ast_node_t& node) : is_none(false), node(node) {}

    inline maybe_node_t(const maybe_node_t& other)
        : is_none(other.is_none), node(other.node) {}
};
...
struct maybe_error_t {
    ...
    inline maybe_error_t(const maybe_node_t& maybe_node)
        : is_error(false), maybe_node(maybe_node) {}

    inline maybe_error_t(const char* error) : is_error(true), error(error) {}
    ...
    inline maybe_error_t(const ast_node_t& node)
        : is_error(false), maybe_node(node) {}
};
...
struct parser_t {
    ...
    inline parser_t(const lexer_t& lexer) : lexer(lexer) {}
    ...
    inline maybe_error_t parse_number() {
        ...
        if (current_token.token == INT_10 || current_token.token == FLOAT_10) {
            // explicitly copy
            const auto parsed_token = parsed_token_t{
            ...
            const auto node = ast_node_t(NUMBER, parsed_token);

            auto result = maybe_node_t(node);
            rsyscall::print_string("ast parsed number: ");
            rsyscall::print_string(result.node.token.data.c_str());
    ...
    inline maybe_error_t parse_product() {
        ...
        for (eat(); current_token.token == MUL || current_token.token == DIV ||
             ...
             eat()) {
            auto parsed_token = current_token;
            ...
            auto result_node = ast_node_t(BINARY_OP, parsed_token);
            result_node.children[0] = ast_node_t(result.maybe_node.node);
    ...
    inline maybe_error_t parse_sum() {
        ...
        rsyscall::print_string("got lhs: ");
        rsyscall::print_string(result.maybe_node.node.token.data.c_str());
        // rsyscall::print_string(" ");
        // rsyscall::print_string(rhs.maybe_node.node.token.data.c_str());
        rsyscall::print_string("\n");
        if (result.is_error) {
            return result;
        }
...
inline void print_ast(const ast_node_t& node, size_t depth) {
    ...
    rsyscall::print_string(" ");
    if (node.token.token == INT_10 || node.token.token == FLOAT_10) {
        auto data = node.token.data;
        data.push_back('\0');
        rsyscall::print_string(data.c_str());
    }
    rsyscall::print_string("\n");
    // rsyscall::print_int((size_t) node.children);
