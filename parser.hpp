...
struct parsed_token_t {
    ...
    // parsed_token_t(const parsed_token_t& parsed_token)
    //     : token(parsed_token.token),
    //       start(parsed_token.start),
    //       end(parsed_token.end),
    //       row(parsed_token.row),
    //       col(parsed_token.col),
    //       data(parsed_token.data) {}
};
...
struct lexer_t {
    ...
    inline parsed_token_t parse_name(size_t start, size_t row, size_t col,
                                     size_t shift = 0) {
        if (shift == 0) current_string.clear();
        ...
        auto parsed_token = parsed_token_t{
            ...
            .row = row,
            .col = col,
            .data = rstd::string(current_string),
        };
    ...
    inline parsed_token_t parse_number() {
        ...
        if (c == '\0' || c != '.') {
            rsyscall::print_string("parsed int: ");
            rsyscall::print_string(current_string.c_str());
        ...
        // rsyscall::print_string("float\n");
        rsyscall::print_string("parsed float: ");
        rsyscall::print_string(current_string.c_str());
        rsyscall::print_string("\n");
        auto parsed_token = parsed_token_t{
            .token = INT_10,
